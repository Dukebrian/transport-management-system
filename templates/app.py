...
def init_db():
    ...
    # 4. Buses Table (Linked to Saccos and Routes)
    conn.execute("""
    ...
    # Bootstrap Default Local Saccos
    cursor = conn.cursor()
...
def admin_dashboard():
    ...
    return render_template('admin.html', routes=routes, buses=buses, subs=subs, payments=payments)
...
def add_bus():
    ...
    conn = get_db_connection()
    try:
        conn.execute("INSERT INTO buses (plate_number, route_id, capacity) VALUES (?, ?, ?)", (plate, route_id, capacity))
        conn.commit()
    except sqlite3.IntegrityError:
...
def passenger_dashboard():
    ...
    SELECT s.*, r.route_name, r.monthly_fare FROM subscriptions s 
    JOIN routes r ON s.route_id = r.route_id WHERE s.passenger_id=?
    """, (p_id,)).fetchall()
